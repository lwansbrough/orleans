...
namespace Orleans.Streams
{
    internal class PersistentStreamProducer<T> : IInternalAsyncBatchObserver<T>
    {
        ...
        {
            return this.queueAdapter.QueueMessageAsync(this.stream.StreamId.Key, this.stream.StreamId.Namespace, item, token, RequestContext.Export(this.serializationManager));
        }

        public Task OnNextBatchAsync(IEnumerable<T> batch, StreamSequenceToken token)
        {
            return this.queueAdapter.QueueMessageBatchAsync(this.stream.StreamId.Key, this.stream.StreamId.Namespace, batch, token, RequestContext.Export(this.serializationManager));
        }
